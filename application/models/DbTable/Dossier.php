<?php
    ...
    class Model_DbTable_Dossier extends Zend_Db_Table_Abstract
    {
        ...
        // Added by George: retourne les dossiers avec commission en salle mais sans date commission
        public function ListeDossiersCommsansDate($idComm)
        {
            $select = "
                    SELECT 
                        dossier.ID_DOSSIER, dossier.OBJET_DOSSIER, dossier.DATEINSERT_DOSSIER, dossier.SERVICEINSTRUC_DOSSIER, CONCAT(utilisateurinformations.PRENOM_UTILISATEURINFORMATIONS, ' ', utilisateurinformations.NOM_UTILISATEURINFORMATIONS) as preventionniste
                    FROM 
                        dossier
                    LEFT JOIN 
                        dossierpreventionniste ON dossierpreventionniste.ID_DOSSIER = dossier.ID_DOSSIER
                    LEFT JOIN
                        utilisateur ON utilisateur.ID_UTILISATEUR = dossierpreventionniste.ID_PREVENTIONNISTE
                    LEFT JOIN
                        utilisateurinformations ON utilisateurinformations.ID_UTILISATEURINFORMATIONS = utilisateur.ID_UTILISATEURINFORMATIONS
                    WHERE  
                        dossier.COMMISSION_DOSSIER = $idComm
                    AND
                        dossier.TYPE_DOSSIER = 1
                    AND
                        dossier.DATECOMM_DOSSIER IS NULL
                    AND
                        dossier.DATEINSERT_DOSSIER BETWEEN NOW()-INTERVAL 180 DAY AND NOW()
                    AND
                        dossier.ABSQUORUM_DOSSIER = 0
                    ORDER BY
                        dossier.DATEINSERT_DOSSIER";
            
            return $this -> getAdapter() -> fetchAll($select);
        }
        
        // Added by George: pour renseigner la reference courrier
        // Increment de la reference courrier
        public function maxReferenceCourrier($year)
        {
