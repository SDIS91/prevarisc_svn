<?php
    class Model_DbTable_DateCommission extends Zend_Db_Table_Abstract
    {
        ...
        public function getNextCommission($idsCommission, $date, $next_date)
        {
            ...
            // George: Restreindre les ordres du jours aux visites en salle et vision sur 3 mois
            $select = "SELECT *
                FROM datecommission d
                LEFT JOIN commission c ON d.COMMISSION_CONCERNE = c.ID_COMMISSION
                WHERE DATE_COMMISSION BETWEEN '".date('Y-m-d', $date)."' AND '".date('Y-m-d', $next_date)."' AND c.ID_COMMISSIONTYPE IN (1,4) AND d.ID_COMMISSIONTYPEEVENEMENT = 1
        ...
        // Pour le moment dans ce fichier pour ne pas modifier le fichier du model des commissions
        public function ListeCommSans()
        {
            $select = "
                    SELECT 
                        ID_COMMISSION, LIBELLE_COMMISSION 
                    FROM 
                        `commission` 
                    WHERE  
                        ID_COMMISSIONTYPE IN (1, 4)
                    ORDER BY
                         LIBELLE_COMMISSION";
