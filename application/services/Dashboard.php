...
class Service_Dashboard
{
    ...
    protected $blocsConfig = array(
        ...
        // commented by george : un pave etudes en attente de traitement au lieu de dossiers non verouilles
        ...
        // added by george:un pave etudes en attente de traitement au lieu de dossiers non verouilles
        'EtudesEnAttente' => array(
            'service' => 'Service_Dashboard',
            'method'  => 'getEtudesEnAttente',
            'acl'     => array('dashboard', 'view_commissions_etudes'),
            'title'   => 'Etudes en attente de traitement par type de commission',
            'type'    => 'etudes',
            'height'  => 'small',
            'width'   => 'small',
        ),
    ...
    // Added by george: fonction pour le bloc getEtudesEnAttente
    protected function getCommSans($idComm) {
        $dbDossierSans = new Model_DbTable_Dossier;
        $dossans = $dbDossierSans -> ListeDossiersCommsansDate($idComm);
        $dossans = array_unique($dossans, SORT_REGULAR);
        return $dossans;
    }
    ...
    // added by george
    public function getEtudesEnAttente($user) {
        $dbComm = new Model_DbTable_DateCommission;
        $commEtudes = $dbComm -> ListeCommSans();
        
        // on récupère pour chaque commission le nombre de dossiers lies
        $commissions = array();
        foreach($commEtudes as $commissionetude)
        {
           $commissions[] = array(
               "id"   => $commissionetude['ID_COMMISSION'],
               "LIBELLE_COMMISSION" => $commissionetude["LIBELLE_COMMISSION"],
               "dossansdate" => $this -> getCommSans($commissionetude['ID_COMMISSION'])
           );
        }
